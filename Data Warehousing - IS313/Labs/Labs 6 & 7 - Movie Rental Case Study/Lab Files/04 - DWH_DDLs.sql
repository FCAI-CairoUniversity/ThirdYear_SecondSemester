...
CREATE TABLE DWH.Fact_Rental (
    rental_key		      int IDENTITY(1,1) PRIMARY KEY,
	rental_id			  INT NOT NULL,
    rental_date_key       int REFERENCES DWH.D_Date(DateKey),
    return_date_key       int REFERENCES DWH.D_Date(DateKey),
    customer_key		  int REFERENCES DWH.D_Customer(customer_key),
    movie_key			  int REFERENCES DWH.D_Movie(movie_key),
    store_key			  int REFERENCES DWH.D_Store(store_key),
...
SELECT * FROM DWH.D_Movie;
SELECT * FROM DWH.D_Store;

select * from DWH.Fact_Rental
